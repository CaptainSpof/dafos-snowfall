            ...
            paths = mkOption {
              ...
              default =
                if config.secrets.environment.template != null then
                  ...
                  (mapAttrsToList (_template-name: value: value.path) config.secrets.environment.templates);
            };
          };
      ...
      mapAttrsToList (
        ...
        (mapAttrsToList (template-name: template: {
          assertion =
            ...
            || (template.source == null && template.text != null);
          message = "dafos.services.vault-agent.services.${service-name}.secrets.environment.templates.${template-name} must set either `source` or `text`.";
        ...
        ++ (mapAttrsToList (file-name: file: {
          ...
          message = "dafos.services.vault-agent.services.${service-name}.secrets.file.files.${file-name} must set either `source` or `text`.";
    ...
    detsys.vaultAgent = {
      ...
      systemd.services = mapAttrs (service-name: value: {
        ...
        secretFiles = {
          ...
          files = mapAttrs (_file-name: value: {
